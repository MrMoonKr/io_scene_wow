...
class Singleton_updater(object):
    ...
    def form_branch_url( self, branch: str ):
    ...
    def check_for_update( self, now=False ):
        ...
        if now == False and self.past_interval_timestamp()==False:
            if self._verbose: print("Aborting check for updated, check interval not reached")
...
class GithubEngine( object ):
    ...
    def form_repo_url( self, updater: Singleton_updater ):
        return "{}{}{}{}{}".format( self.api_url, "/repos/", updater.user, "/", updater.repo )

    def form_tags_url( self, updater: Singleton_updater ):
        if updater.use_releases:
            return "{}{}".format( self.form_repo_url( updater ), "/releases" )
        else:
            return "{}{}".format( self.form_repo_url( updater ), "/tags" )

    def form_branch_list_url( self, updater: Singleton_updater ):
        return "{}{}".format( self.form_repo_url( updater ), "/branches" )

    def form_branch_url( self, branch: str, updater: Singleton_updater ):
